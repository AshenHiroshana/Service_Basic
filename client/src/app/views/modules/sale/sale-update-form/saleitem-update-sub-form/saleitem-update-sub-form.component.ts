...
@Component({
  selector: 'app-saleitem-update-sub-form',
  templateUrl: './saleitem-update-sub-form.component.html',
  styleUrls: ['./saleitem-update-sub-form.component.scss'],
  providers: [
    {
...
export class SaleitemUpdateSubFormComponent extends AbstractSubFormComponent<Saleitem> implements OnInit{
  ...
  getDeleteConfirmMessage(saleitem: Saleitem): string {
    return 'Are you sure to remove \u201C ' + saleitem.id + ' \u201D from saled items?';
  }

  getUpdateConfirmMessage(saleitem: Saleitem): string {
    if (this.isFormEmpty){
      return 'Are you sure to update \u201C\u00A0' + saleitem.id + '\u00A0\u201D\u00A0?';
    }

    return 'Are you sure to update \u201C\u00A0' + saleitem.id + '\u00A0\u201D and discard existing form data\u00A0?';
  }
