...
export class VehicleDetailComponent extends AbstractComponent implements OnInit {
  ...
  async delete(): Promise<void>{
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      width: '300px',
      data: {message: this.vehicle.code + '-' + this.vehicle.no}
    });
    ...
    dialogRef.afterClosed().subscribe( async result => {
      ...
      try {
        await this.vehicleService.delete(this.selectedId);
  ...
  async loadData(): Promise<any> {
    ...
    if (this.vehicle.photo) {
      const photoOb = await this.vehicleService.getPhoto(this.selectedId);
